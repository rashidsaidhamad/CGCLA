...
// Unit Modal (same structure as DepartmentModal but labeled for Units)
const UnitModal = ({
  showAddModal,
  editingUnit,
  newUnit,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-md w-full shadow-2xl">
        <div className="p-6">
          ...
          <form onSubmit={(e) => {
            e.preventDefault();
            editingUnit ? handleUpdateUnit() : handleAddUnit();
          }}>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Unit Name *
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="Enter unit name (e.g. ICT Unit)"
              />
            </div>
...
const DepartmentManagement = ({ user }) => {
  ...
  const [selectedTab, setSelectedTab] = useState('departments'); // 'departments' | 'units'
  ...
  const fetchUnits = async () => {
    try {
      const response = await fetch(`${API_BASE}/auth/units/`, { headers: getHeaders() });
  ...
  const handleAddUnit = async () => {
    try {
      const response = await fetch(`${API_BASE}/auth/units/`, {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({ name: newUnit.name, description: newUnit.description })
      });
  ...
  const handleUpdateUnit = async () => {
    try {
      const response = await fetch(`${API_BASE}/auth/units/${editingUnit.id}/`, {
        method: 'PUT',
        headers: getHeaders(),
        body: JSON.stringify({ name: newUnit.name, description: newUnit.description })
      });
  ...
  const handleDeleteUnit = async (unitId) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/auth/units/${unitId}/`, {
        method: 'DELETE',
        headers: getHeaders(),
      });
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
  ...
  const handleUpdateDepartment = async () => {
    try {
      const response = await fetch(`${API_BASE}/auth/departments/${editingDepartment.id}/`, {
        method: 'PUT',
        headers: getHeaders(),
        body: JSON.stringify({ name: newDepartment.name }),
      });
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl shadow-lg p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold mb-2">{selectedTab === 'departments' ? 'Department Management' : 'Unit Management'}</h1>
            <p className="text-blue-100 text-lg">
              {selectedTab === 'departments'
                ? 'Organize and manage your company departments'
                : 'Register and manage organizational units (e.g. ICT Unit)'}
            </p>
          </div>
          <div className="flex items-center space-x-6">
            ...
            <div className="bg-white/10 rounded-lg p-1 flex items-center">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedTab === 'departments' ? 'bg-white text-blue-700' : 'text-white/80 hover:bg-white/20'}`}
              >
                Departments
              </button>
              <button
                onClick={() => setSelectedTab('units')}
                className={`ml-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedTab === 'units' ? 'bg-white text-blue-700' : 'text-white/80 hover:bg-white/20'}`}
              >
                Units
              </button>
            </div>
            ...
            <div className="text-center">
              <div className="text-3xl font-bold">{selectedTab === 'departments' ? departments.length : units.length}</div>
              <div className="text-blue-100">Total {selectedTab === 'departments' ? 'Departments' : 'Units'}</div>
            </div>

            <button
              onClick={() => selectedTab === 'departments' ? setShowAddModal(true) : setShowAddUnitModal(true)}
            ...
            >
              ...
              {selectedTab === 'departments' ? 'Add Department' : 'Add Unit'}
            </button>
          </div>
        </div>
      </div>

      {/* Content: Departments or Units */}
      {selectedTab === 'departments' ? (
        <>
          {/* Search Bar */}
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center space-x-4">
              ...
              <div className="text-sm text-gray-500">
                {units.filter(u => u.name.toLowerCase().includes(unitSearchTerm.toLowerCase())).length} unit{units.filter(u => u.name.toLowerCase().includes(unitSearchTerm.toLowerCase())).length !== 1 ? 's' : ''} found
              </div>
            </div>
          </div>
          ...
          {/* Units Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {units.filter(u => u.name.toLowerCase().includes(unitSearchTerm.toLowerCase())).map((unit) => (
              <div key={unit.id} className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-shadow p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl flex items-center justify-center">
                    <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M10 2a2 2 0 00-2 2v2H6a2 2 0 00-2 2v2h12V8a2 2 0 00-2-2h-2V4a2 2 0 00-2-2zM4 14v2a2 2 0 002 2h8a2 2 0 002-2v-2H4z" />
                    </svg>
                  </div>
                  <div className="flex space-x-1">
                    <button
