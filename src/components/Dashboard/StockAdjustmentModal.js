  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            New Unit Price (TSh) - Optional
          </label>
          <input
            type="number"
            min="0"
            step="0.01"
            value={stockAdjustment.unit_price}
            onChange={(e) => setStockAdjustment({...stockAdjustment, unit_price: parseFloat(e.target.value) || 0})}
            className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
            placeholder={`Current: ${selectedItem?.unit_price || 0} TSh`}
          />
          <p className="text-xs text-gray-500 mt-1">Leave empty to keep current price</p>
        </div>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <button
            ...
            disabled={!stockAdjustment.quantity || !stockAdjustment.reason || !stockAdjustment.date}
